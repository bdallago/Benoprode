...
import { Trophy, Users, Swords, UserPlus, Check, X, Clock, BookOpen, Loader2, PenSquare, Medal } from 'lucide-react';
...
export default function Profile({ user, profileId }: ProfileProps) {
  ...
  useEffect(() => {
    if (globalLeagues) {
      const userLeagues = globalLeagues.filter((l: any) => l.members?.includes(targetUserId) || l.createdBy === targetUserId);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="py-4">
        ...
        <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-sm mt-8">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="flex-1">
                <div className="flex justify-between text-sm mb-2 text-gray-600 dark:text-gray-200 font-medium">
                  ...
                  <span>{badges.length} de {BADGES.length} ({Math.round((badges.length / BADGES.length) * 100)}%)</span>
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 mb-6">
                  <div className="bg-blue-500 h-3 rounded-full transition-all duration-1000" style={{ width: `${(badges.length / BADGES.length) * 100}%` }}></div>
                </div>
                ...
                <div className="flex flex-wrap gap-4">
                  {badges.length > 0 ? badges.map((badge: any) => badge && (
              ...
              <div className="w-full md:w-80 bg-gray-50 dark:bg-gray-900/50 rounded-xl p-5 border border-gray-200 dark:border-gray-700">
                ...
                <div 
                  ...
                  onClick={() => setIsAllBadgesModalOpen(true)}
                >
                  {BADGES.filter(b => !badgeIds.includes(b.id)).slice(0, 4).map(badge => {
            ...
            {duelsList.length > 0 ? (
              ...
              <div className="bg-white dark:bg-gray-800 p-8 rounded-xl border border-dashed border-gray-300 dark:border-gray-600 text-center flex flex-col items-center justify-center">
                ...
                <p className="text-gray-500 dark:text-gray-400 max-w-sm mb-6 text-sm">
                  Desafiá a un amigo y apuesten quién sabe más en esta fecha. ¡El que acierta más, gana puntos extra!
                </p>
                {isOwnProfile && (
                  <Button onClick={() => setActiveTab('friends')} className="bg-blue-600 hover:bg-blue-700 font-bold shadow-sm">
